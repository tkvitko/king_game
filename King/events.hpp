...
public:
    ...
    float change_countryman_by_koef_per_countrymen = 0.0;
    
    Event() {};
    
    Event(std::string text_, int change_balance_, int change_countryman_, int change_revenue_from_tourism_percentage_) {
        text=text_;
        change_balance=change_balance_;
        change_countryman=change_countryman_;
        change_revenue_from_tourism_percentage=change_revenue_from_tourism_percentage_;
    };
    
    Event(std::string text_, int change_balance_, int change_countryman_,
          int change_revenue_from_tourism_percentage_, int change_balance_by_koef_per_countryman_) {
        text=text_;
        change_balance=change_balance_;
        change_countryman=change_countryman_;
        change_revenue_from_tourism_percentage=change_revenue_from_tourism_percentage_;
        change_balance_by_koef_per_countryman=change_balance_by_koef_per_countryman_;
    };
    
    Event(std::string text_, int change_balance_, int change_countryman_,
          int change_revenue_from_tourism_percentage_, float change_countryman_by_koef_per_countrymen_) {
        text=text_;
        change_balance=change_balance_;
        change_countryman=change_countryman_;
        change_revenue_from_tourism_percentage=change_revenue_from_tourism_percentage_;
        change_countryman_by_koef_per_countrymen=change_countryman_by_koef_per_countrymen_;
    };
...
public:
    Quizes() {
        ...
        // todo harvest
//        Event good_event_6 = Event("Хорошие новости! Согласно анализу почвы, новое удобрение сработало. Говорить о результатах пока, конечно, рано, но, по словам специалистов, в будущем году можно ожидать двукратного прироста урожая.",
//                              0, 0, 0);
//        Event bad_event_6 = Event("Эти горе-учёные опять что-то напутали. По словам приглашённых независимых специалистов, плодородность почвы упала в 2 раза, так что в ближайший год нас ждёт неурожай.",
//                              0, 0, 0);
...
//        this->quizes.push_back(quiz_6);
        ...
        // todo cost of life
//        Event good_event_7 = Event("Поздравляю, господин премьер-министр! Испытательные прогоны увенчались полным успехом, а регулярные рейсы начнутся с будущего года.",
//                              0, 0, 0);
//        Event bad_event_7 = Event("Проект провалился. Почва в месте прокладки оказалась неподходящей. Потери казны составили 20000 ролодов.",
//                              -20000, 0, 0);
...
public:
    Events() {
        ...
        // todo countryman for the next year
